...
弾幕はAPIレスポンス後すぐにロードされる。ARIB字幕はデフォルトで逐次ロード（抽出途中でも表示開始）され、`arib_reload_interval=0` で従来通り抽出完了後の一括ロードに変更可能。両方揃った時点でマージトラックが作られ、`default_track` の設定に従って自動的に切り替わる。
...
| `arib_reload_interval` | `1` | ARIB字幕の逐次リロード間隔秒（`0` = 逐次リロード無効、抽出完了後に一括ロード） |
